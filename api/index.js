...
// =============================================================
// MIDDLEWARE: extraer rol y vet_id del header de sesión simulado
// En producción real esto vendría de un JWT verificado.
// Aquí usamos headers simples para demostrar RLS en la defensa.
//
// Headers esperados:
//   X-User-Role: veterinario | recepcion | admin
//   X-Vet-Id:   1 | 2 | 3   (solo cuando role=veterinario)
// =============================================================
function extractSession(req, res, next) {
    const role  = req.headers['x-user-role']  || 'admin';
    const vetId = req.headers['x-vet-id']     || null;

    // Validar que el rol sea uno de los permitidos (evita inyección de rol)
    const validRoles = ['veterinario', 'recepcion', 'admin'];
    if (!validRoles.includes(role)) {
        return res.status(401).json({ error: 'Rol inválido.' });
    }

    // Si es veterinario, validar que vet_id sea un entero positivo
    if (role === 'veterinario') {
        const parsedVetId = parseInt(vetId, 10);
        if (isNaN(parsedVetId) || parsedVetId <= 0) {
            return res.status(401).json({ error: 'vet_id inválido para rol veterinario.' });
        }
...
// =============================================================
// HELPER: obtener cliente de BD según el rol de la sesión
// Cada rol usa el usuario de PostgreSQL correspondiente.
// =============================================================
async function getDbClient(session) {
    return getClientForRole(session.role, session.vetId);
}
...
//
// HARDENING (línea crítica):
//   pool.query('... WHERE m.nombre ILIKE $1', [`%${nombre}%`])
//
// El input del usuario NUNCA se concatena. El driver pg envía
// `nombre` como parámetro separado del SQL — el servidor de BD
// lo trata como dato, no como código. Un input como
//   ' OR '1'='1
// llega literalmente como string de búsqueda, no como SQL.
// =============================================================
app.get('/api/mascotas', async (req, res) => {
    ...
    try {
        ...
        // Establecer contexto de vet si aplica (para RLS)
        if (req.session.role === 'veterinario') {
            await client.query('SELECT set_vet_context($1)', [req.session.vetId]);
        }
...
app.get('/api/mascotas/:id', async (req, res) => {
    ...
    try {
        ...
        if (result.rowCount === 0) return res.status(404).json({ error: 'Mascota no encontrada o sin acceso.' });
...
// Esta es la consulta más costosa — se cachea en Redis.
// Estrategia: cache-aside con TTL de 5 minutos.
// Invalidación explícita cuando se aplica una vacuna nueva.
// =============================================================
app.get('/api/vacunacion-pendiente', async (req, res) => {
    ...
    try {
        ...
        // Si Redis falla, continuamos con la BD (degraded mode)
    ...
    try {
        ...
        // 3. Guardar en caché con TTL de 5 minutos (300 segundos)
...
app.post('/api/vacunas-aplicadas', async (req, res) => {
    ...
    try {
        ...
        // INSERT parametrizado — nunca se concatena input
        const result = await client.query(
...
app.post('/api/citas', async (req, res) => {
    ...
    // Validar que fecha_hora sea un timestamp válido
    const fechaDate = new Date(fecha_hora);
    if (isNaN(fechaDate.getTime())) {
        return res.status(400).json({ error: 'fecha_hora no es una fecha válida.' });
    }
    ...
    try {
        ...
        // Llamar al procedure — todos los parámetros son posicionales
        const result = await client.query(
...
// ---- POST /api/auth/login -------------------------------------
// Simula el login. En producción esto verificaría contraseña
// y emitiría un JWT. Aquí simplemente devuelve la sesión.
// =============================================================
app.post('/api/auth/login', async (req, res) => {
    const { usuario, password } = req.body;

    // Mapa de usuarios de prueba (en producción vendría de la BD)
    const usuarios = {
        'vet_lopez':          { role: 'veterinario', vetId: 1, nombre: 'Dr. Fernando López Castro' },
        'vet_garcia':         { role: 'veterinario', vetId: 2, nombre: 'Dra. Sofía García Velasco' },
        'vet_mendez':         { role: 'veterinario', vetId: 3, nombre: 'Dr. Andrés Méndez Bravo' },
        'usuario_recepcion':  { role: 'recepcion',   vetId: null, nombre: 'Personal de Recepción' },
    ...
    res.json({
        ...
        // En producción esto sería un JWT firmado
...
app.listen(PORT, () => {
    console.log(`[SERVER] Clínica Vet API corriendo en http://localhost:${PORT}`);
    console.log(`[SERVER] Hardening: queries parametrizadas con driver pg`);
    console.log(`[SERVER] RLS: contexto de vet via SET LOCAL app.current_vet_id`);
});
